  ...
  render() {
    return (
      <CustomGridList>
        <CustomGrid>
            <TextCard
                title="Thoughts on model development"
            >
              ...
              <p>
                Data should always flow into the model automatically.  This can done through web-sockets or REST hooks.  This is often used with external data like <a href='https://research.stlouisfed.org/fred2/'>FRED</a>.  Data can also flow through by calling data directly from a database. The database should be created in such a way that data types are guaranteed.  For example, primary and foreign keys should be enforced, data should be labeled "not null" where appropriate.  During development the data environment should be investigated so that no surprises show up after the model has been placed in production.
                </p>
            </TextCard>
        </CustomGrid>
        ...
        <CustomGrid>
            <TextCard
                title="Model Risk"
            >
                <p>Emanuel Derman (of <a href='http://www.emanuelderman.com/media/faj-one_factor_model.pdf'>Black-Derman-Toy</a> fame) anticipated the current scrutiny on model risk by a few decades. <a href='http://www.emanuelderman.com/media/gs-model_risk.pdf'>His now classic paper on model risk</a> was published over ten years before the "great recession" and in many ways offers a more concise and sophisticated approach to model risk than that provided by <a href='http://www.federalreserve.gov/bankinforeg/srletters/sr1107a1.pdf'>SR 11-7</a>.  Model risk is logically exposited and easily understood.  However, his most practical contribution may be his model taxonomy.</p>
             <p>
